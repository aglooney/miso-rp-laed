...
<<<<<<< HEAD
    ...
    P_ED, Shed_ED, LMP_ED, TLMP_ED, rup_ED, rupp_Ed, rdw_ED, rdwp_Ed = ED_with_error(data_ed, N_g, 2, N_T, load_factor, ramp_factor, solver, sigma, rho, rng, error_type="student-t")
    #P_LAED, Shed_LAED, Curt_LAED, TLMP, LLMP, R_LAED, RP_LAED = LAED_No_Errors(data_laed, N_g, N_t, N_T,load_factor, ramp_factor, solver)
    P_LAED, Shed_LAED, TLMP_LAED, LLMP_LAED, R_LAED, RP_LAED = LAED_with_error(data_laed, N_g, N_t, N_T, load_factor, ramp_factor, solver, sigma, rho, rng, error_type="student-t")
    ...
    plt.ylabel("Power (MW)")
    plt.legend()
    plt.title("Comparison of Load Shedding under RP (10-minute) and LAED")
    plt.show()
    ...
    plt.xlabel("Time")
    plt.ylabel("Power (MW)")
    plt.title("Comparison of Generator Values under RP (10-minute) and LAED")
    plt.legend()
    plt.show()
